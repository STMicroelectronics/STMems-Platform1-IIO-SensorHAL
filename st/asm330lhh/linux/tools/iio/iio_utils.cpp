...
static const char *iio_sf_filename = "sampling_frequency";
...
static const char *iio_device_name = "iio:device";
...
int iio_utils::sysfs_write_int(char *file, int val)
{
	FILE *fp;
...
int iio_utils::sysfs_write_scale(char *file, float val)
{
	FILE *fp;
...
int iio_utils::sysfs_read_scale(char *file, float *val)
{
	FILE *fp;
...
int iio_utils::sysfs_write_str(char *file, char *str)
{
	FILE *fp;
...
int iio_utils::sysfs_read_str(char *file, char *str, int len)
{
	FILE *fp;
	int err = 0;
...
int iio_utils::sysfs_read_int(char *file, int *val)
{
	FILE *fp;
	int ret;
...
int iio_utils::enable_channels(const char *device_dir, bool enable)
{
...
int iio_utils::get_device_by_name(const char *name)
{
	struct dirent *ent;
	...
	for (ent = readdir(dp); ent; ent = readdir(dp)) {
		if (strlen(ent->d_name) <= strlen(iio_device_name) ||
		    !strcmp(ent->d_name, ".") || !strcmp(ent->d_name, ".."))
			continue;

		if (strncmp(ent->d_name, iio_device_name, strlen(iio_device_name)) == 0) {
			numstrlen = sscanf(ent->d_name + strlen(iio_device_name), "%d", &number);
			fnamelen = numstrlen + strlen(iio_dir) + strlen(iio_device_name);
			if (fnamelen > DEVICE_IIO_MAX_FILENAME_LEN)
				continue;

			sprintf(dfilename, "%s%s%d/name", iio_dir, iio_device_name, number);
...
int iio_utils::enable_sensor(char *device_dir, bool enable)
{
	...
	sprintf(enable_file, "%s/%s", device_dir, iio_buffer_enable);

	return sysfs_write_int(enable_file, enable);
}

int iio_utils::get_sampling_frequency_available(char *device_dir,
			struct device_iio_sampling_freq_avail *sfa)
{
	...
	err = sprintf(sf_filaname, "%s/%s", device_dir, iio_sfa_filename);
	if (err < 0)
		return err;
...
int iio_utils::get_fifo_length(const char *device_dir)
{
	int ret;
	int len;
	...
	ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN,
		       "%s/%s", device_dir, iio_hw_fifo_length);
	if (ret < 0)
		return -ENOMEM;
	...
	ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN,
		       "%s/%s", device_dir, iio_buffer_length);
	if (ret < 0)
		return -ENOMEM;
	...
	ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN,
		       "%s/%s", device_dir, iio_hw_fifo_enabled);
	if (ret < 0)
		return -ENOMEM;
...
int iio_utils::set_sampling_frequency(char *device_dir, unsigned int frequency)
{
	int ret;
	char tmp_filaname[DEVICE_IIO_MAX_FILENAME_LEN];
	...
	ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN,
		       "%s/%s", device_dir, iio_sf_filename);

	return ret < 0 ? -ENOMEM : sysfs_write_int(tmp_filaname, frequency);
}

int iio_utils::set_hw_fifo_watermark(char *device_dir,  unsigned int watermark)
{
	int ret;
	char tmp_filaname[DEVICE_IIO_MAX_FILENAME_LEN];
	...
	ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN,
		       "%s/%s", device_dir, iio_hw_fifo_watermark);

	return ret < 0 ? -ENOMEM : sysfs_write_int(tmp_filaname, watermark);
}

int iio_utils::hw_fifo_flush(char *device_dir)
{
	int ret;
	char tmp_filaname[DEVICE_IIO_MAX_FILENAME_LEN];
	...
	ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN,
		       "%s/%s", device_dir, iio_hw_fifo_flush);
	if (ret < 0)
		return -ENOMEM;
...
int iio_utils::set_scale(const char *device_dir, float value,
...
int iio_utils::get_scale(const char *device_dir, float *value,
			 device_iio_chan_type_t device_type)
{
	int ret;
	char tmp_filaname[DEVICE_IIO_MAX_FILENAME_LEN];
...
int iio_utils::get_type(struct device_iio_channel_info *channel,
...
int iio_utils::get_scale_available(const char *device_dir,
...
int iio_utils::support_injection_mode(const char *device_dir)
{
	int err;
	char injectors[30];
	...
	/* Check string len */
	if (strlen(device_dir) + 
		strlen(iio_injection_mode_enable) + 1 > DEVICE_IIO_MAX_FILENAME_LEN)
		return -1;

	sprintf(tmp_filaname, "%s/%s", device_dir, iio_injection_mode_enable);
	err = sysfs_read_int(tmp_filaname, &err);
	if (err < 0) {
		sprintf(tmp_filaname, "%s/%s", device_dir, iio_injection_sensors_filename);
...
int iio_utils::set_injection_mode(const char *device_dir,
				  bool enable)
{
	char tmp_filaname[DEVICE_IIO_MAX_FILENAME_LEN];
	int ret = 0;
	...
	ret = snprintf(tmp_filaname, DEVICE_IIO_MAX_FILENAME_LEN,
		       "%s/%s", device_dir, iio_injection_mode_enable);
	if (ret < 0)
		return -ENOMEM;
...
int iio_utils::inject_data(const char *device_dir, unsigned char *data, 
