/*
 * STMicroelectronics GeomagVector Sensor Class
 *
...
GeomagVector::GeomagVector(HWSensorBaseCommonData *data, const char *name,
		...
		unsigned int hw_fifo_len, float power_consumption, bool wakeup) :
			...
			SENSOR_TYPE_GEOMAGNETIC_ROTATION_VECTOR, hw_fifo_len, power_consumption)
{
...
GeomagVector::~GeomagVector()
{

}

int GeomagVector::Enable(int handle, bool enable, bool lock_en_mutex)
{
	return HWSensorBaseWithPollrate::Enable(handle, enable, lock_en_mutex);
}

void GeomagVector::ProcessData(SensorBaseData *data)
{
	...
	sensor_event.acceleration.x = half_to_float(data->processed[0]);
	sensor_event.acceleration.y = half_to_float(data->processed[1]);
	sensor_event.acceleration.z = half_to_float(data->processed[2]);
