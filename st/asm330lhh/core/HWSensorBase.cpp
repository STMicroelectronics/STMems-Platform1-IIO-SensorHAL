...
HWSensorBase::HWSensorBase(HWSensorBaseCommonData *data, const char *name,
		int handle, STMSensorType sensor_type, unsigned int hw_fifo_len,
		float power_consumption) : SensorBase(name, handle, sensor_type)
{
	...
	info.power = power_consumption;
	info.fifoMaxCount = hw_fifo_len;
	...
	case 0:
		...
		console.error(GetName() + std::string(": injection mode available, sensor is an injector."));
#endif /* CONFIG_ST_HAL_DEBUG_INFO */
		info.flags |= DATA_INJECTION_MASK;
		injection_mode = SENSOR_INJECTOR;
		break;

	case 1:
#if (CONFIG_ST_HAL_DEBUG_LEVEL >= ST_HAL_DEBUG_INFO)
		console.error(GetName() + std::string(": injection mode available, sensor is injected."));
#endif /* CONFIG_ST_HAL_DEBUG_INFO */
		info.flags |= DATA_INJECTION_MASK;
	...
	default:
#if (CONFIG_ST_HAL_DEBUG_LEVEL >= ST_HAL_DEBUG_INFO)
		console.error(GetName() + std::string(": injection mode not available."));
#endif /* CONFIG_ST_HAL_DEBUG_INFO */
		info.flags &= ~DATA_INJECTION_MASK;
		break;
	}
			   ...
			   int64_t timeout, bool lock_en_mutex)
{
	...
	if (timeout < INT64_MAX) {
		if ((info.fifoMaxCount == 0) && (timeout > 0))
			return -EINVAL;
	} else
		return 0;
	...
	if (info.fifoMaxCount > 0) {
		buf_len = timeout / FREQUENCY_TO_NS(1);
		if (buf_len > info.fifoMaxCount)
			buf_len = info.fifoMaxCount;
...
#if (CONFIG_ST_HAL_DEBUG_LEVEL >= ST_HAL_DEBUG_INFO)
	ALOGD("\"%s\": changed pollrate to timeout=%" PRIu64 "ms (sensor type: %d).", info.name,
			(uint64_t)NS_TO_MS((uint64_t)timeout), info.type);
#endif /* CONFIG_ST_HAL_DEBUG_INFO */
	...
	if (GetStatus(false)) {
		...
		if ((GetMinTimeout(false) > 0) &&
			...
			for (i = 0; i < dependencies.num; i++)
				dependencies.sb[i]->FlushData(info.handle, true);
			...
			ProcessFlushData(info.handle, 0);
	} else
		goto unlock_mutex;
							...
							power_consumption)
{
	...
	info.maxRateHz = max_sampling_frequency;
#if (CONFIG_ST_HAL_ANDROID_VERSION > ST_HAL_KITKAT_VERSION)
	info.minRateHz = min_sampling_frequency;
#endif /* CONFIG_ST_HAL_ANDROID_VERSION */
}
				       ...
				       int64_t timeout, bool lock_en_mutex)
{
...
#if (CONFIG_ST_HAL_ANDROID_VERSION > ST_HAL_KITKAT_VERSION)
		if (period_ns > (((int64_t)FREQUENCY_TO_US(info.minRateHz)) * 1000))
			period_ns = FREQUENCY_TO_US(info.minRateHz) * 1000;
#endif /* CONFIG_ST_HAL_ANDROID_VERSION */

		if ((period_ns < (((int64_t)FREQUENCY_TO_US(info.maxRateHz)) * 1000)) &&
		    (period_ns > 0))
			period_ns = FREQUENCY_TO_US(info.maxRateHz) * 1000;
	}
	...
	if (message)
		ALOGD("\"%s\": changed pollrate to %.2fHz, timeout=%" PRIu64 "ms (sensor type: %d).",
			info.name, NS_TO_FREQUENCY((float)(uint64_t)min_pollrate_ns),
			(uint64_t)NS_TO_MS((uint64_t)min_timeout_ns), info.type);
#endif /* CONFIG_ST_HAL_DEBUG_INFO */
...
int HWSensorBaseWithPollrate::FlushData(int handle, bool lock_en_mutex)
{
	...
	if (GetStatus(false)) {
		...
		if ((GetMinTimeout(false) > 0) && (GetMinTimeout(false) < INT64_MAX)) {
			...
			ProcessFlushData(info.handle, utils.getTime());
	} else
		goto unlock_mutex;
...
void HWSensorBaseWithPollrate::WriteDataToPipe(int64_t hw_pollrate)
{
	...
	if (ValidDataToPush(sensor_event.timestamp)) {
		...
		if (((samples_counter % decimator) == 0) || odr_changed) {
			...
			ALOGD("\"%s\": pushed data to android: timestamp=%" PRIu64 "ns real_pollrate=%" PRIu64 " (sensor type: %d).", info.name, sensor_event.timestamp, current_real_pollrate, info.type);
