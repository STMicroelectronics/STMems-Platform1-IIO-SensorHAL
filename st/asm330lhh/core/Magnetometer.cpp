...
Magnetometer::Magnetometer(HWSensorBaseCommonData *data, const char *name,
		...
		unsigned int hw_fifo_len, float power_consumption, bool wakeup) :
			HWSensorBaseWithPollrate(data, name, sfa, handle,
						 STMSensorType::MAGNETOMETER, hw_fifo_len, power_consumption)
{
	...
	if (wakeup)
		info.flags |= SENSOR_FLAG_WAKE_UP;
#else /* CONFIG_ST_HAL_ANDROID_VERSION */
	(void)wakeup;
#endif /* CONFIG_ST_HAL_ANDROID_VERSION */

	info.resolution = GAUSS_TO_UTESLA(data->channels[0].scale);
	info.maxRange = info.resolution * (pow(2, data->channels[0].bits_used - 1) - 1);
}
