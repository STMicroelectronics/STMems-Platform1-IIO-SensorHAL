...
int SWSensorBase::AddSensorDependency(SensorBase *p)
{
	...
	if (info.fifoMaxCount == 0)
		info.fifoMaxCount = p->GetMaxFifoLenght();
	else {
		if (p->GetMaxFifoLenght() < (int)info.fifoMaxCount)
			info.fifoMaxCount = p->GetMaxFifoLenght();
	}
	...
	if (dependency_resolution)
		info.resolution = dependecy.resolution;

	if (dependency_range)
		info.maxRange = dependecy.maxRange;

	if (dependency_delay) {
		if (dependency_ID == id_sensor_trigger) {
			info.maxRateHz = dependecy.maxRateHz;
#if (CONFIG_ST_HAL_ANDROID_VERSION > ST_HAL_KITKAT_VERSION)
			if ((info.minRateHz == 0) || (info.minRateHz > dependecy.minRateHz))
				info.minRateHz = dependecy.minRateHz;
#endif /* CONFIG_ST_HAL_ANDROID_VERSION */
		}
	}

	if (dependency_name)
		info.name = dependecy.name;
...
void SWSensorBase::ProcessFlushData(int handle, int64_t timestamp)
{
	...
	if (info.type == STMSensorType::INTERNAL_LIB_TYPE) {
...
void SWSensorBaseWithPollrate::WriteDataToPipe(int64_t hw_pollrate)
{
	...
	if (((samples_counter % decimator) == 0) || odr_changed) {
		...
		ALOGD("\"%s\": pushed data to android: timestamp=%" PRIu64 "ns (sensor type: %d).", info.name, sensor_event.timestamp, info.type);
#endif /* CONFIG_ST_HAL_DEBUG_LEVEL */
	}
