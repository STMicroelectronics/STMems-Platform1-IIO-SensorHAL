...
SensorBase::SensorBase(const char *name, int handle, STMSensorType type)
{
	int i, err, pipe_fd[2];
	...
	case STMSensorType::ACCELEROMETER:
		sensor_event.type = SENSOR_TYPE_ACCELEROMETER;
		break;
	case STMSensorType::GYROSCOPE:
		sensor_event.type = SENSOR_TYPE_GYROSCOPE;
		break;
	default:
		break;
	}
	...
	info.handle = handle;
	info.type = type;
	info.vendor = std::string("STMicroelectronics");
	info.version = 1;
...
int SensorBase::Enable(int handle, bool enable, bool lock_en_mutex)
{
	...
	if ((handle == info.handle) && (enable == GetStatusOfHandle(handle)))
	...
	if ((enable && !GetStatus(false)) || (!enable && !GetStatusExcludeHandle(handle))) {
		...
		for (i = 0; i < dependencies.num; i++) {
			err = dependencies.sb[i]->Enable(info.handle, enable, true);
			if (err < 0)
...
restore_enable_dependencies:
	while (i > 0) {
		i--;
		dependencies.sb[i]->Enable(info.handle, !enable, true);
	}
...
int SensorBase::SetDelay(int handle, int64_t period_ns, int64_t timeout, bool lock_en_mutex)
{
	...
	if ((timeout > 0) && (timeout < INT64_MAX) && (info.fifoMaxCount == 0))
		return -EINVAL;
	...
	for (i = 0; i < (int)dependencies.num; i++) {
		err = dependencies.sb[i]->SetDelay(info.handle, GetMinPeriod(false), GetMinTimeout(false), true);
		if (err < 0)
...
restore_delay_dependencies:
	...
	for (i--; i >= 0; i--)
		dependencies.sb[i]->SetDelay(info.handle, GetMinPeriod(false), GetMinTimeout(false), true);
...
const std::vector<STMSensorType>& SensorBase::GetDepenciesTypeList(void)
{
...
int SensorBase::AddSensorDependency(SensorBase *p)
{
	...
	const struct Info &dependecy = p->getSTMSensor();
	info.power += dependecy.power;

#if (CONFIG_ST_HAL_ANDROID_VERSION > ST_HAL_KITKAT_VERSION)
	//sensor_dependency_wake_flag = (dependecy_data.flags & SENSOR_FLAG_WAKE_UP);
	//if (!sensor_dependency_wake_flag)
	//	info.flags &= ~sensor_dependency_wake_flag;
#endif /* CONFIG_ST_HAL_ANDROID_VERSION */
...
const struct SensorBase::Info& SensorBase::getSTMSensor(void)
{
	return info;
}
...
void SensorBase::WriteFlushEventToPipe()
{
	...
	flush_event_data.meta_data.sensor = info.handle;
...
void SensorBase::ProcessData(SensorBaseData *data)
{
	...
	if (data->flush_event_handle == info.handle)
		WriteFlushEventToPipe();

	for (i = 0; i < push_data.num; i++)
		push_data.sb[i]->ReceiveDataFromDependency(info.handle, data);
}
...
void *SensorBase::ThreadCalibWork(void *context)
{
	SensorBase *mypointer = (SensorBase *)context;

	mypointer->ThreadCalibTask();

	return mypointer;
}

void SensorBase::ThreadCalibTask()
{
	pthread_exit(NULL);
}
...
int SensorBase::hasCalibChannels()
{
	return 0;
}
