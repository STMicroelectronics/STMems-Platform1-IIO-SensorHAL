...
protected:
	...
	mag_cal_data_t mag_static_offset[2];

public:
	SensorBase(const char *name, int handle, STMSensorType type);
	virtual ~SensorBase();
	...
	const struct SensorBase::Info& getSTMSensor(void);
	const std::vector<STMSensorType>& GetDepenciesTypeList(void);
	...
	static void *ThreadCalibWork(void *context);
	virtual void ThreadCalibTask();
